...
var (
	...
	HashIdsEncodeError = NewError(10011, "HashID加密失败")
	HashIdsDecodeError = NewError(10012, "HashID解密失败")
	RBACError          = NewError(10013, "暂无访问权限")
	RedisConnectError  = NewError(10014, "Redis连接失败")
	MySQLConnectError  = NewError(10015, "MySQL连接失败")
	WriteConfigError   = NewError(10016, "写入配置文件失败")
	SendEmailError     = NewError(10017, "发送邮件失败")
	MySQLExecError     = NewError(10018, "SQL执行失败")
	GoVersionError     = NewError(10019, "Go版本不满足要求")
	EmptyTokenError    = NewError(10020, "Token令牌不存在")
	InvalidTokenError  = NewError(10021, "Token令牌不正确，请重新登录")
	TokenTimeoutError  = NewError(10022, "Token令牌过期，请重新登录")
	...
	AuthorizedCreateAPIError = NewError(10106, "创建调用方API地址失败")
	AuthorizedListAPIError   = NewError(10107, "获取调用方API地址列表失败")
	AuthorizedDeleteAPIError = NewError(10108, "删除调用方API地址失败")
