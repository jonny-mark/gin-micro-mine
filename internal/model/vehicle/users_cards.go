...
type VehiclesModel struct {
	...
	PlateColor       int8       `gorm:"column:plate_color;type:tinyint;not null;default:0" json:"plate_color"`                       // 车牌颜色：0、蓝色 1、黄色 2、黑色 3、白色 4、渐变绿色 5、黄绿双拼色 6、蓝白渐变色 7、临时牌照 9、未确定 11、绿色 12、红色
	...
	AllowReactivate  int8       `gorm:"column:allow_reactivate;type:tinyint unsigned;not null;default:0" json:"allow_reactivate"`    // 是否允许重新激活
	...
	Status           int8       `gorm:"column:status;type:tinyint unsigned;not null;default:0" json:"status"`                        // 卡片状态 0:正常 2:挂失 3:补办中 4:作废 8:注销中 9:已注销
	TollPayChannel   int8       `gorm:"column:toll_pay_channel;type:tinyint unsigned;not null;default:1" json:"toll_pay_channel"`    // 通行扣费支付渠道 1、我方自己发起的扣费 2、米大师发起的扣费 3、招行代扣
	ServiceStatus    int8       `gorm:"column:service_status;type:tinyint;not null;default:0" json:"service_status"`                 // 是否开启售后权限
	VoidStatus       int8       `gorm:"column:void_status;type:tinyint;not null;default:0" json:"void_status"`                       // 是否允许用户注销 0:不允许 1:允许
	Manufacturer     int8       `gorm:"column:manufacturer;type:tinyint unsigned;not null;default:0" json:"manufacturer"`            // 设备商类型（埃特斯1、金溢2、聚力3、万集4、成谷5）
	DeviceType       int8       `gorm:"column:device_type;type:tinyint unsigned;not null;default:0" json:"device_type"`              // 设备类型[0-普通 1-可充电设备]
	...
	ShowRefundBtn    int8       `gorm:"column:show_refund_btn;type:tinyint unsigned;not null;default:0" json:"show_refund_btn"`      // 是否显示退款按钮
	...
	SettleCycle      int8       `gorm:"column:settle_cycle;type:tinyint;not null;default:1" json:"settle_cycle"`                     // 结算周期：1日结2周结3月结
	HasChangeSign    int8       `gorm:"column:has_change_sign;type:tinyint(1);not null;default:0" json:"has_change_sign"`            // 是否已修改签约[0-未改签 1-已改签]
	IsFront          int8       `gorm:"column:is_front;type:tinyint unsigned;not null;default:0" json:"is_front"`                    // 是否前装[0-默认 1-前装]
